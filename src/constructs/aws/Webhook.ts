...
export class Webhook extends AwsConstruct {
  ...
  constructor(
    ...
    const resolvedConfiguration = Object.assign(
      {},
      WEBHOOK_DEFAULTS,
      configuration
    );
    if (
      resolvedConfiguration.insecure &&
      resolvedConfiguration.authorizer !== undefined
    ) {
    ...
    if (!resolvedConfiguration.insecure) {
      const lambda = Function.fromFunctionArn(
        ...
        "LambdaAuthorizer",
        Fn.getAtt(
          provider.naming.getLambdaLogicalId(`${id}Authorizer`),
          "Arn"
        ).toString()
      );
  ...
  private appendFunctions(): void {
    const resolvedWebhookConfiguration = Object.assign(
      {},
      WEBHOOK_DEFAULTS,
      this.configuration
    );
    ...
    this.provider.addFunction(
      `${this.id}Authorizer`,
      resolvedWebhookConfiguration.authorizer
    );
  }
  ...
  private async getUrl(): Promise<string | undefined> {
    const apiEndpoint = await this.provider.getStackOutput(
      this.apiEndpointOutput
    );
